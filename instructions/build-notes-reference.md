...
## 2026-07-05 update — Assets map, Gen AI, 3D, DOT Love Philippines
- New `website/assets/img/ph-provinces.svg`: clickable map, 119 admin-1 areas. Source: @highcharts/map-collection 2.3.3 ph-all.geo.json (Natural Earth-derived) fetched via npm registry (only allowlisted host); SVG generated by script, hover = accent #ffd400, each province links to `provinces/<slug>.html` (target=_top, hrefs are ../../ relative because SVG loads via <object>). **Province pages do not exist yet** — clicks 404 until provinces/ is built.
- index.html: new sections ASSETS (#assets, embeds map via <object>), GEN AI (#genai, 3 draft cards awaiting owner's video links), 3D·AR·VR (#three-d, photogrammetry + Gatorade cards real, AR/VR draft slots). Nav + section dots extended. `.phmap` CSS appended to main.css.
- New story page `stories/dot-love-philippines.html` via generator PAGES entry. Campaign facts web-verified (launched 2023-06-27, replaced "It's More Fun", "love" from social-listening survey). Budots-specific deliverables under the campaign = draft marker.
